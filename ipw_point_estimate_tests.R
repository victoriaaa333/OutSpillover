  ...
  for(ll in 1:l){    
    ...
    for (j in 1:k){
      ...
      if (Con_type == "mixed"){
        ...
        # TODO: check this
        #H_list <- neigh_coefs_oncont1(weights_df, H, G, neighinfo, A, a)
    ...
    for (j in 1:k) {
      ...
      if (Con_type == "mixed"){
        ...
        # grp_est[ , p, j, ] <- apply(as.matrix(x1), 2, neigh_means_oncont1,
        #                              overall_coef = ova_coef_est_H[ , , j],
        #                              X_type = X_type, N = N)
        grp_est[ , p, j, ] <- apply(as.matrix(x1), 2, neigh_means_oncont1_eachg,
        ...
        #grp_est[, p, j, ] <- group_means_null_1(weights_df, H, G, A, a)
      }
    }
...
## Group coefficient (weights outside) ##
...
# 2. test where weights is outside the response
ipw_point_estimates_mixed_test2 <- function(H, G, A, weights, X = NULL, x0 = NULL, 
                                           neighinfo = NULL, x1= NULL, X_type = NULL,
                                           Con_type = "No-con"){
  ## Necessary Bits ##
  grps     <- dimnames(weights)[[1]]
...
# 3. test for without additional terms when weights is inside the response
ipw_point_estimates_mixed_test3 <- function(H, G, A, weights, X = NULL, x0 = NULL, 
                                            neighinfo = NULL, x1= NULL, X_type = NULL,
                                            Con_type = "No-con"){
  ## Necessary Bits ##
  grps     <- dimnames(weights)[[1]]
  ...
  for(ll in 1:l){    
    ...
    for (j in 1:k){
      ...
      if (Con_type == "mixed"){
        ...
        # TODO: check this
        H_list <- neigh_coefs_oncont3(ind_est_df, H, G, neighinfo, A, a)
        #H_list <- neigh_coefs_oncont_old(ind_est_df, H, G, neighinfo, A, a)
        coef_est_H[ , , j] <- as.matrix(H_list[[1]])
        ova_coef_est_H[ , , j] <- as.matrix(H_list[[2]])
      }else if (Con_type == "group"){
        G_list <- group_coefs_oncont2(ind_est_df, H, G, X, X_type, x0, A, a)
        coef_est_G[ , , j] <- as.matrix(G_list[[1]])
        ova_coef_est_G[ , , j] <- as.matrix(G_list[[2]])
      }
    }
  ...
  for(ll in 1:l){    
    ...
    for (j in 1:k) {
      ...
      if (Con_type == "mixed"){
        ...
        #grp_est_overall[, p, j, ] <- group_means_null0(ind_est_df, G, A, a)
        grp_est_overall[, p, j, ] <- group_means_null(ind_est_df, G, A, a)
        grp_est[, p, j, ] <- group_means_null(ind_est_df, G, A, a)
        # grp_est_overall[, p, j, ] <- group_means_null_0(weights_est_df, H, G, A, a)
        # grp_est[, p, j, ] <- group_means_null_1(weights_est_df, H, G, A, a)
      }
    }
...
# 5. test where weights is outside the response without extra terms (with group_est)
