var_para <- function(reg_coef, a, t, X_reg, H, weights_ind, A, G, cat_ind){
  len_n <- dim(X_reg)[2] - 1
  N <- length(unique(G))
  oval_coef_a <- reg_coef[, a, t]
  #X_reg <- cbind(1, X_num)
  X_fit <- apply(X_reg, 1, function(x) sum(x * oval_coef_a))
  ...
  #weight_set0 <- weights_ind[, , a, t]
  #weight_set0[is.na(cat_ind)] = 0
  #psid_mat <- as.matrix(replicate(dim(X_reg)[2], weight_set0)) * X_reg
  psid_mat <- as.matrix(replicate(dim(X_reg)[2], (weights_ind[, , a, t]))) * X_reg
  psid_data <- as.data.frame(cbind(G, A, psid_mat, X_reg, cat_ind))
...
sig_effect  <- function(psi_group1, psid_mat_grp1, psi_group2, psid_mat_grp2, len_n){
  ...
  #V_mat <- var(as.matrix(psi_group[,-1]))
  U_mat <- rbind(cbind(apply(psid_mat_grp1, 1:2, mean, na.rm = TRUE), matrix(0,len_n+1,len_n+1)),
                 cbind(matrix(0,len_n+1,len_n+1), apply(psid_mat_grp2, 1:2, mean, na.rm = TRUE)))
  U_mat_inv <- solve(U_mat)
  var_mat <- U_mat_inv %*% V_mat %*% t(U_mat_inv)/N
  var_mat
}  

var_outcome <- function(G, A,  var_mat, cat_ind, X_reg, X_mean = NULL){ # X_reg, t1, cat_ind, 
  if(is.null(X_mean)){
    #X_reg <- cbind(1, X_num)
...
var_effect <- function(G, A, var_mat, cat_ind, X_reg, X_mean = NULL){ #X_reg, t1, t2, cat_ind
  if (is.null(X_mean)){
    #X_reg <- cbind(1, X_num)
    X_data <- as.data.frame(cbind(G, A, X_reg))
    X_data_t1 <- X_data[cat_ind == 1,]#[A == t1, ]
