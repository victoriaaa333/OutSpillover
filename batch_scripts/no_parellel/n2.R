...
print("Starting numerical for mixed")
result_n2 <- foreach(i = 1:200, .combine="c") %do% {
  ...
  Y = apply(cbind(df$A, df$treated_neigh, df$interaction1, df$interaction2, 
                  df$interaction3, df$interaction4), 1, 
...
saveRDS(result_n2, "check_results/mixed_model_num_var_newvarpara.RDS")
