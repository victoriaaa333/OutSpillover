# We try to treat weights in numerator and denominator differently,
# with numerator modifying response and denominator not

# 4. test where weights is outside the response without extra terms
ipw_point_estimates_mixed_test5 <- function(H, G, A, weights, X = NULL, x0 = NULL, 
  ...
  if(is.matrix(weights)){
    ...
    weights_numerators <- array(c(weight_numerator(A, G, numerator_alphas),1), dim=c(N, 1, k))
    # TODO: if numerator_alphas is more than 1, rewrite function weight_numerator
    predictors <- NULL
  } else {
  ...
  for(ll in 1:l){    
    ...
    weights_trt_numerator <- array(dim= c(length(A), p, k))
    weights_trt_denominator <- array(dim= c(length(A), p, k))

    for(pp in 1:p){
      for (kk in 1:k) {
        weights_trt[ , pp, kk] <- apply(as.array(G), 1, function(x) weights[,pp,kk][x])
        weights_trt_numerator[ , pp, kk] <- apply(as.array(G), 1, function(x) weights_numerators[,pp,kk][x])
        weights_trt_denominator[ , pp, kk] <- weights_trt[ , pp, kk]/weights_trt_numerator[ , pp, kk] # taking the inverse
        weights_trt_numerator[ , pp, kk] <- weights_trt_numerator[ , pp, kk] * (A == a)/
    ...
    coef_est_H_deno <- array(dim= c(N, p, k))
    ...
    for (j in 1:k){
      ...
      weights_num_df <- weights_trt_numerator[, , j]
      weights_den_df <- weights_trt_denominator[, , j]
      if (Con_type == "mixed"){
        ...
        H_list <- neigh_coefs_oncont5(weights_num_df, weights_den_df, H, G, neighinfo, A, a)
        coef_est_H[ , , j] <- as.matrix(H_list[[1]])
        ova_coef_est_H[ , , j] <- as.matrix(H_list[[2]])
        coef_est_H_deno[, , j] <- as.numeric(H_list[[3]])
    ...
    for (j in 1:k) {
      ...
      if (Con_type == "mixed"){
        ...
        grp_est[ , p, j, ] <- apply(as.matrix(x1), 2, neigh_means_oncont5,
                                    overall_coef = ova_coef_est_H[ , , j],
                                    X_type = X_type, N = N, w_den = coef_est_H_deno[, , j])
        # change w_den
        # grp_est[ , p, j, ] <- apply(as.matrix(x1), 2, neigh_means_oncont3,
        #                             cond_coefs = coef_est_H[ , , j],
        #                             X_type = X_type)
      }else if (Con_type == "group"){
        grp_est[ , p, j, ] <- apply(as.matrix(x0), 2, group_means_oncont1, 
...
neigh_coefs_oncont5 <- function(weights_num_df, weights_den_df, H, G, neighinfo, A, a){
  neighX = neighinfo$neighX
  colnames(neighX) <- paste("neighX", colnames(neighX), sep = "_")
  
  if (!is.null(neighinfo) && !is.null(x1)){
    weighted_H <- weights_num_df*H
    group_df <- as.data.frame(cbind(weighted_H, weights_den_df, H, G, A, neighX))
    colnames(group_df)[1] <- 'w_H'
    colnames(group_df)[2] <- 'w_den' 
    
    fits_df <- as.data.frame(group_df[group_df$A == a,])
    fits <- lmList(as.formula(paste("w_H ~ ", paste(colnames(fits_df)[-1:-5], 
                                                  collapse= "+ "), "| G")), 
                   data = fits_df, weights = w_den)
    cond_coef <- as.matrix(coef(fits))
    # TODO: group means for coef
    
    overall_fits <- lm(as.formula(paste("w_H ~ ", paste(colnames(fits_df)[-1:-5],
                                                      collapse= "+ "))), 
                       data = fits_df, weights = w_den)
    overall_coef <- as.matrix(coef(overall_fits))
    coef <- list(cond_coef, overall_coef, mean(fits_df$w_den, na.rm = TRUE))
...
# TODO: numerical_alpha default to the other value?
weight_numerator <- function(A, G, numerator_alpha){
  numerator <- by(A, INDICES = G, simplify = FALSE, 
          FUN = function(x) {
            prod(numerator_alpha^x * (1-numerator_alpha)^(1-x))})
  return(as.matrix(as.numeric(numerator)))
}

neigh_means_oncont5 <-function(overall_coef, X_type, x1, N, w_den){
  x1_num <- as.numeric(x1[X_type == "N"])
  lenn <- length(x1_num)
  cond_coefs <- matrix(rep(overall_coef, N), nrow = N, byrow = TRUE)
  if (lenn > 0){
    cond_group_means <- (cond_coefs[,1] + as.matrix(cond_coefs[,-1])%*%as.matrix(x1_num)) *w_den
  }else{
    cond_group_means <- cond_coefs
    # TODO:?
  }
