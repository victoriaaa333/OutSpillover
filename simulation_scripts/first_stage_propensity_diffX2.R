...
for (i in 1:5) {
  ...
  X4 <- rnorm(length(A), mean = 0.5, sd = 0.5)
  ...
  try({
    ...
    parameters = c(rep(0, 4), parameters2)
    ...
    obj_N <- ipw_propensity_variance_regression(parameters = parameters,
                                                ...
                                                X_type = c("N"), # X_type
                                                Con_type = "neigh")
    result_N = rbind(result_N, obj_N)}, silent = TRUE)
}

saveRDS(result_G, "cluster_results/first_propensity_group2.RDS")
saveRDS(result, "cluster_results/second_propensity_nocon2.RDS")
saveRDS(result_N, "cluster_results/second_propensity_neigh2.RDS")
saveRDS(result_G1, "cluster_results/second_propensity_group12.RDS")
