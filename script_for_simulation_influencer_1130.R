...
sd(aa1$estimate) #3.236499
mean(aa1$std.error) #3.144438
mean(aa1$estimate) #-13.17396
mean(aa1_est1) #-13.17396
saveRDS(aa1, "../kaggle/working/inf_plain.RDS")
...
sd(bb2$estimate) # 3.86935
mean(bb2$std.error) # 5.060228
mean(bb2$estimate) # -21.37539
# - (5 + 7 + 9) = -21

sd(cc2$estimate) # 4.932064
mean(cc2$std.error) # 4.932535
mean(cc2$estimate) # -20.83747
# - (5 + 7 + 9) = -21
saveRDS(aa2, "../kaggle/working/inf_binary1.RDS")
saveRDS(bb2, "../kaggle/working/inf_binary2.RDS")
saveRDS(cc2, "../kaggle/working/inf_binary3.RDS")
...
sd(aa3$estimate) #8.143858
mean(aa3$std.error)# 6.80677
mean(aa3$estimate) # -13.97713
# - (5 + 0.5*7 + 0.5*9) = -13
mean(est3n) # -16.80774
# TODO: check if average over groups isn't unbiased for neighbors?
saveRDS(aa3, "../kaggle/working/inf_cont1.RDS")
saveRDS(est3n, "../kaggle/working/inf_cont1_pest.RDS")

sd(bb3$estimate) #2.394977
mean(bb3$std.error) # 2.175651
mean(bb3$estimate) # -8.252241
#-(5 + 0.5*(7+9)) = - 7.5
mean(est3g) # -7.35624
saveRDS(bb3, "../kaggle/working/inf_cont2.RDS")
saveRDS(est3g, "../kaggle/working/inf_cont2_pest.RDS")
