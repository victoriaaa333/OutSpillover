...
ipw_point_estimates_cont <- function(H, G, A, weights, X = NULL, x0 = NULL){
  ...
  for(ll in 1:l){    
    ...
    grp_est[, p, ] <- apply(ind_est, 3, group_means_oncont, G, X, x0, A, a)
...
group_means_oncont <- function(ind_est_df, G, X, x0, A, a){
  ...
  # group_df <- as.data.frame(group_df[group_df$ind_est != 0,]) # revise this
...
# output the regression coefficients for each group and overall.
# plot x0 against effect through grid of x0